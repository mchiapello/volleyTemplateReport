    ...
    if (by == "player") {
        ...
        vr_pts <- y %>% group_by(.data$set_number) %>%
            dplyr::summarize(Ser = sum(.data$evaluation_code == "#" & .data$skill == "Serve" & .data$team %in% team_select, na.rm = TRUE),
                             ...
                             Tot = sum(.data$evaluation_code == "#" & .data$skill %in% c("Serve", "Attack", "Block")),
                                 # BP = sum(.data$evaluation_code == "#" & .data$skill %in% c("Serve", "Attack", "Block") & .data$serving_team == team_select),
                                 Nerr = sum((.data$evaluation %eq% "Error" & .data$skill %in% c("Serve", "Reception", "Attack", if (!as_for_datavolley) "Set", if (!as_for_datavolley) "Freeball")) | (!as_for_datavolley & .data$evaluation %eq% "Invasion" & .data$skill %eq% "Block") | (.data$evaluation %eq% "Blocked" & .data$skill %eq% "Attack")),
                                 'W-L' = .data$Tot - .data$Nerr,
...
t1 %>% 
    ...
    cols_label(
        ...
        Nerr = html("<span style='color: #ff0000'>Err</span>"),
        `W-L` = html("D"),
        Tot.y = html("<strong>Tot</strong>"),
        Err.x = html("<span style='color: #ff0000'>Err</span>"),
        Pts.x = html("Pts"),
        Tot.x.x = html("<strong>Tot</strong>"),
        Err.y = html("<span style='color: #ff0000'>Err</span>"),
        Tot.y.y = html("<strong>Tot</strong>"),
        Err.x.x = html("<span style='color: #ff0000'>Err</span>"),
        Pts.y = html("Pts"),
        Tot = html("<strong>Tot</strong>"),
        Err.y.y = html("<span style='color: #ff0000'>Err</span>"),
        Punto = html("Pts")) %>% 
    cols_align(
    align = "center") %>% 
...
t2 %>% 
    ...
    cols_label(
        ...
        `W-L` = html("D"),
        Tot...6 = html("<strong>Tot</strong>"),
        Err...7 = html("<strong>Err</strong>"),
        Pts...8 = html("Pts"),
        Tot...11 = html("<strong>Tot</strong>"),
        Err...12 = html("<strong>Err</strong>"),
        Tot...16 = html("<strong>Tot</strong>"),
        Err...17 = html("<strong>Err</strong>"),
        Blo = html("Muto"),
        Pts...19 = html("Pts"),
        Tot...21 = html("<strong>Tot</strong>"),
        Err...22 = html("<strong>Err</strong>"),
        Punto = html("Pts")) %>% 
    cols_align(
    align = "center") %>% 
